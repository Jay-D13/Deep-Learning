...
# https://www.youtube.com/watch?v=a4Yfz2FxXiY (explains really well)
...
def alphas_betas_sequences_helper(beta_start, beta_end, T):
    ...
    alphas_cumprod_prev = F.pad(alphas_cumprod, (1, 0), value=1.)[:-1]                  # WRITE CODE HERE: Right shifts \bar{\alpha}_t; with first element as 1.
...
def q_sample(x_start, t, coefficients, noise=None):
    ...
    sqrt_alphas_cumprod = coefficients[0]
    sqrt_one_minus_alphas_cumprod = coefficients[1]
...
def p_losses(denoise_model, x_start, t, coefficients, noise=None):
    ...
    x_noisy = q_sample(x_start, t, coefficients, noise=noise) # WRITE CODE HERE: Obtain the noisy image from the original images x_start, at times t, using the noise noise.
    predicted_noise = denoise_model(x_noisy, t) # WRITE CODE HERE: Obtain the prediction of the noise using the model.
    
    loss = F.smooth_l1_loss(predicted_noise, noise) # WRITE CODE HERE: Compute the huber loss between true noise generated above, and the noise estimate obtained through the model.
...
def t_sample(timesteps, batch_size, device):
    ...
    ts = torch.randint(0, timesteps, (batch_size,), device=device).long() # WRITE CODE HERE: Randommly sample a tensor of size (batch_size,) where entries are independently sampled from [0, ..., timesteps-1]()
    return ts